<template>
  <div class="page">
    <div class="page-header"><h2>历史对话</h2></div>
    <div v-for="item in list" :key="item.id" class="glass-card item-card" @click="router.push({ name: 'HistoryChatDetail', params: { id: item.id } })">
      <div class="item-main">
        <span class="item-title">{{ item.title }}</span>
        <span class="item-meta">{{ item.msg_count || 0 }} 条消息 · {{ item.provider || 'AI' }} · {{ formatDate(item.updated_at) }}</span>
      </div>
      <span class="item-del" @click.stop="handleDelete(item)">✕</span>
    </div>
    <div class="glass-card empty" v-if="!loading && list.length === 0"><p>暂无历史对话</p></div>
  </div>
</template>
...
const router = useRouter(); const list = ref([]); const loading = ref(false)
const onLoad = async () => { try { const res = await request.get('/history/chats'); list.value = res.data.list } catch {} }
const handleDelete = async (item) => { try { await showConfirmDialog({ title: '删除对话', message: `确定要删除「${item.title}」吗？删除后不可恢复。` }) } catch { return }; try { await request.delete(`/history/chats/${item.id}`); list.value = list.value.filter(i => i.id !== item.id) } catch { showToast('删除失败') } }
